        ...
        <div class="window" id="window-view-container" :style="{maxHeight: maxHeight + 'px', width: width +'px'}">
           ...
           <div class="loading" v-if="dataArray && dataArray.length==0">
            ...
            <ScrollLoader :minHeight="minHeight" @scroll-to-top="refresh" @scroll-to-botton="infinite" class="container-main" v-if="dataArray && dataArray.length>0" :style="{maxHeight: maxHeight-50 + 'px'}">
                <div class="message">
                    <ul>
                        <li v-for="(message, index) in dataArray" :key="message.id" class="an-move-right">
                            ...
                            <div class="main" v-else>
                                <img class="avatar" width="45" height="45" :src="message.sender.avatar">
                                <!-- 文本 -->
                                <div class="text" v-emotion="message.content" v-if="message.type=='text'"></div>

                                <!-- 图片 -->
                                <div class="text" v-else-if="message.type=='image' || message.type=='emoji'">
                                    <img :src="message.content" class="image" alt="聊天图片">
                                </div>
								<!-- 视频 -->
								<div class="text" v-else-if="message.type=='video'">
								    <iframe width="250px" height="130px" src="http://player.youku.com/embed/XMzcwNTY3NTM2MA" frameborder=0 allowfullscreen></iframe>
								</div>
								<!-- 语音 -->
								<div class="text" v-else-if="message.type=='voice'">
								   <!-- <audio width="250px" height='70px' src="http://img95.699pic.com/audio/463/632/5aefca2f85c71_all.mp3"  controls="controls" preload="auto">
									</audio> -->
									<m-audio :src="message.content"></m-audio>
								</div>
								<!-- 文档1-->
								<div class="text" v-else-if="message.type=='attachment'">
									<a :href="message.content">{{message.extra.filename}}</a>
								</div>
								<!-- 文档2 -->
								<div class="text" v-else-if="message.type=='weblink'">
									<a :href="message.content">{{message.extra.title}}</a>
								</div>
                                <!-- 其他 -->
